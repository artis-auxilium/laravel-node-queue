...
module.exports = function help(settings) {
  ...
  shell.help = function shellHelp(req, res) {
    ...
    if (command) {
      ...
      res.yellow('Help:').ln();
      res.print(pad('', 2)).white(route.description).ln().ln();
      res.yellow('Usage:').ln();
      res.print(pad('', 2)).white(route.name);
      ...
      each(route.keys.params, function eachParam(param) {
        ...
        args += pad('', 2) + pad('\x1b[32m' + param.key, argPadding);
        if (param.help) {
      ...
      each(route.keys.options, function eachOption(option) {
        res.print(pad('', 2)).green(pad(option.key, argPadding));
        if (option.help) {
    ...
    each(routes, function eachRoute(rte) {
      ...
      if (rte.description) {
        res.print(pad('', 2)).green(routeText).white(rte.description).ln();
      }
    });
    return res.prompt();
  };
